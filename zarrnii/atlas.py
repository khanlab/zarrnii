...
def _extract_2d_slice(
    ...
    """Extract 2D slice from a ZarrNii image.

    Args:
        image: ZarrNii image to slice
        slice_position: Position of slice in physical coordinates
        plane: Plane orientation - 'axial', 'sagittal', or 'coronal'
        patch_size: Optional maximum patch size in physical units (e.g., mm)
            If specified, crops the slice to this size centered at the slice position

    Returns:
        Tuple of (slice_data, slice_metadata) where:
            - slice_data: 2D numpy array of the slice
            - slice_metadata: Dict with keys:
                - 'extent': (x_min, x_max, y_min, y_max) in physical coordinates
                - 'plane': plane orientation
                - 'position': slice position in physical coordinates
                - 'aspect': aspect ratio for plotting
    ...
    affine_transform = image.get_affine_transform()

    # Convert slice position to voxel index
    # Create a point at the slice position
...
def visualize_atlas_regions(
    ...
    This function creates 2D slice visualizations centered on specified atlas regions,
    ...
    Args:
        atlas: ZarrNii or ZarrNiiAtlas image containing the atlas data
    ...
    Examples:
        ...
        >>>
        >>> # Visualize specific regions without centroids
    ...
    for region_id in region_ids:
        ...
        for plane in planes:
            ...
            # Extract 2D slice
            try:
                slice_data, slice_metadata = _extract_2d_slice(
                    atlas, slice_position, plane, max_patch_size
                )
            except Exception as e:
                warnings.warn(
                    f"Failed to extract slice for region {region_id}, plane {plane}: {e}"
                )
                continue

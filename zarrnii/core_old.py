...
class ZarrNii:
    ...
    def axes(self) -> Optional[List[Dict]]:
        """Axes metadata - derived from NgffImage."""
        # Convert NgffImage dims to OME axes format for compatibility
        if hasattr(self.ngff_image, 'axes') and self.ngff_image.axes:
            return self.ngff_image.axes
        else:
            # Generate default axes from dims
            axes = []
    ...
    def coordinate_transformations(self) -> Optional[List[Dict]]:
        ...
        # Convert NgffImage scale/translation to OME coordinate transformations format
        transforms = []
        ...
        for dim in self.ngff_image.dims:
            if dim in self.ngff_image.scale:
                scale_dict[dim] = self.ngff_image.scale[dim]
            if dim in self.ngff_image.translation:
                translation_dict[dim] = self.ngff_image.translation[dim]
        
        if scale_dict:
            transforms.append({
                "type": "scale",
                "scale": [scale_dict.get(dim, 1.0) for dim in self.ngff_image.dims]
            })
        
        if any(v != 0.0 for v in translation_dict.values()):
            transforms.append({
                "type": "translation", 
                "translation": [translation_dict.get(dim, 0.0) for dim in self.ngff_image.dims]
            })
    ...
    def omero(self) -> Optional[Dict]:
        ...
        # This would need to be stored separately or derived if needed
        return None
    ...
    def from_darr(
        ...
        Parameters:
            ...
            name (str): Name for the NgffImage.
        ...
        if affine is None:
            affine_matrix = orientation_to_affine(orientation, spacing, origin)
        elif isinstance(affine, AffineTransform):
    ...
    def from_array(
        ...
        """
        Creates a ZarrNii instance from a NumPy array.

        Parameters:
            array (np.ndarray): Input NumPy array.
            affine (AffineTransform or np.ndarray, optional): Affine transform.
            chunks (str or tuple): Chunking strategy for the dask array.
            orientation (str, optional): Orientation string (default: "RAS").
            axes_order (str): The axes order of the input array (default: "XYZ").
            spacing (tuple, optional): Voxel spacing along each axis (default: (1, 1, 1)).
            origin (tuple, optional): Origin point in physical space (default: (0, 0, 0)).
            name (str): Name for the NgffImage.

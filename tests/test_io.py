...
def test_xyz_orientation_backwards_compatibility(tmp_path):
    ...
    loaded_znimg_default = ZarrNii.from_ome_zarr(str(zarr_path_default), orientation="LPI")
...
def test_reverse_orientation_string():
    ...
    for orientation in ["RAS", "LPI", "RAI", "LPS"]:
        reversed_twice = reverse_orientation_string(reverse_orientation_string(orientation))
        assert reversed_twice == orientation
    
    # Test error cases
    with pytest.raises(ValueError, match="Orientation string must be exactly 3 characters"):

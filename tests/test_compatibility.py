...
class TestCompatibilityMethods:
    ...
    def test_round_trip_zarrnii_to_ngff_and_back(self, sample_zarrnii):
        ...
        assert_array_equal(sample_zarrnii.darr.compute(), zarrnii_back.darr.compute())

        # Check affine preservation (approximately, due to floating point)
        assert_array_almost_equal(
            sample_zarrnii.affine.matrix, zarrnii_back.affine.matrix, decimal=10
        )
    ...
    def test_round_trip_ngff_to_zarrnii_and_back(self, sample_ngff_image):
        ...
        # Check data preservation
        assert_array_equal(sample_ngff_image.data.compute(), ngff_back.data.compute())

        # Check scale preservation
        for dim in ["z", "y", "x"]:
            assert_array_almost_equal(
                sample_ngff_image.scale[dim], ngff_back.scale[dim], decimal=10
            )
